...
class MIDI_Mix(ControlSurface):
    __doc__ = " Script for MIDI_Mix in APC emulation mode "

    _active_instances = []
    def _combine_active_instances():
        track_offset = 0
        scene_offset = 0
        for instance in MIDI_Mix._active_instances:
    ...
    def _do_combine(self):
        if self not in MIDI_Mix._active_instances:
            MIDI_Mix._active_instances.append(self)
            MIDI_Mix._combine_active_instances()


    def _do_uncombine(self):
        if ((self in MIDI_Mix._active_instances) and MIDI_Mix._active_instances.remove(self)):
